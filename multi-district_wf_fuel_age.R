...
# districts <- c("albany")
timing <- vector(mode = "character")
for(district in districts){
  cli::cli_alert_info("Working on {district}...hang tight")
  tic(district)
  ...
  fa_mat_nom <- paste0(tdir, district, "_fuel_age_area_matrix_", minyr, "-", maxyr, ".csv")
  ...
  wf_mat_nom <- paste0(tdir, district, "_wild_fire_fuel_age_area_matrix_",
                       minyr, "-", maxyr, ".csv")
  ...
  of_mat_nom <- paste0(tdir, district,"_other_fire_fuel_age_area_matrix_", minyr, "-", maxyr, ".csv")

  readr::write_csv(offa_mat, file = of_mat_nom)

  out <- toc(log = TRUE)
  timing <- c(timing, out$callback_msg)
}

timing_df <- tibble::tibble(string = timing) %>%
  rowwise() %>%
  mutate(district = gsub(":", "", strsplit(string, " ")[[1]][1]),
         time_m = as.numeric(strsplit(string, " ")[[1]][2])/60) %>%
  select(-string) %>%
  write_csv("./district_processing_times.csv")
